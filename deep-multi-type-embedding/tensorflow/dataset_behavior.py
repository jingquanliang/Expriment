...
class BehaviorDatasetManager:
    ...
    def read_itemlist_file(self, itemlist_file):
        ...
        for itypeidx, itemindicesset in itypeidx2itemindicesset_dict.items():
            ...
            '''itemindicesset代表所有itypeidx这个类型的项目集合'''

            '''Build alias sampler for uniform sampling of item indices in a given type'''
            item_indices_probs = np.zeros(self.number_of_items) #把所有的项目都置为零
            for item_ind in itemindicesset:
                item_indices_probs[item_ind] = 1 #只是把这个类型的置为1
            item_indices_probs = item_indices_probs / np.sum(item_indices_probs)  # Normalize probs,注意，这里是里面的元素相加，在util函数中，是乘以元素个数，所有会出现有的元素值大于1的情况

            item_indices_sampler = utils.AliasMethod(prob_lst=item_indices_probs) #注意，这里的item_indices_sampler是一个object，不是一个具体的数组
            # print('####item_indices_sampler###' * 30)
            # print(item_indices_sampler)
            # print('###item_indices_sampler####' * 30)
            # exit()
